...
public class DialogueManager {
	...
	public void draw(Graphics g) {
		...
		if (debugDialogueCounter == 0) {
			g.drawString("Test sentence 1", 20, (int)(GamePanel.PANEL_HEIGHT * 2.0 / 3.0) + 40);
		}
		else if (debugDialogueCounter == 1) {
			g.drawString("Test sentence 2", 20, (int)(GamePanel.PANEL_HEIGHT * 2.0 / 3.0) + 40);
		}
		else if (debugDialogueCounter == 2) {
			g.drawString("Test sentence 3", 20, (int)(GamePanel.PANEL_HEIGHT * 2.0 / 3.0) + 40);
		}
	...
	public GameState update(KeyboardInput keyboard) {
		if (inDialogue) {
			if (keyboard.keyPressed(KeyEvent.VK_ENTER)) {
				debugDialogueCounter++;
				if (debugDialogueCounter > 2) {
					debugDialogueCounter = 0;
					inDialogue = false;
				}
			}
		}
